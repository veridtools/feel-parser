...
import type { AstNode, ContextEntry, FeelType, Loc, ParseError } from './ast.js';
import {
  BP,
  KNOWN_NAME_PREFIXES,
  KNOWN_NAMES,
...
class Parser {
  ...
  private expect(type: TokenType): Token {
    const tok = this.peek();
    if (tok.type !== type) {
      throw new Error(`Expected ${type} but got ${tok.type} (${tok.value}) at pos ${tok.start}`);
    }
    return this.advance();
  }
  ...
  // Parse a complete expression at the given binding power
  ...
  private parsePrefix(): AstNode {
    ...
    switch (tok.type) {
      ...
      default:
        throw new Error(`Unexpected token: ${tok.type} (${tok.value}) at pos ${tok.start}`);
    }
  }
  ...
  private parseInfix(left: AstNode, _bp: number): AstNode {
    ...
    switch (tok.type) {
      ...
      case TokenType.Dot: {
        ...
        // Handle multi-word property names like "time offset" or user-defined names
      ...
      case TokenType.In: {
        // x in (a, b, c) → unary test list; x in [1..10] / (1..10] → range; x in expr → single test
        let test: AstNode;
        if (this.check(TokenType.LParen)) {
          this.advance(); // consume (
          // Peek: if it starts with .. it's an open range start; otherwise unary test list
          if (this.check(TokenType.DotDot)) {
            // (..end) — unbounded open range
            this.advance();
            const end = this.parseRangeEnd();
            test = {
              type: 'RangeLiteral',
            ...
            // Parse first expression at BP.Range to stop before ..
            ...
            if (this.check(TokenType.DotDot)) {
              ...
              // Unary test list (possibly single element)
      ...
      default:
        throw new Error(`Unexpected infix token: ${tok.type}`);
    }
  }
  ...
  private parseList(): AstNode {
    ...
    if (this.check(TokenType.DotDot)) {
      // [..end] — unbounded start, [ = start-included is irrelevant
      this.advance();
      const end = this.parseRangeEnd();
      const endIncluded = end.endIncluded;
      return {
        type: 'RangeLiteral',
        startIncluded: true,
        endIncluded,
    ...
    // Parse first element, stopping BEFORE consuming DotDot
    const first = this.parseExpr(BP.Range); // DotDot BP=35 is NOT > 35, so we stop
  ...
  // Parse the end of a range after consuming '..' — returns the end node and whether end is included
  private parseRangeEnd(): { node: AstNode; endIncluded: boolean } {
    ...
    // ] or [ = included (closed); ) = excluded (open); European: [ at end means exclusive
    ...
    if (this.match(TokenType.LBracket)) return { node, endIncluded: false }; // European: [1..10[ = [1..10)
    throw new Error(`Expected range end delimiter ] ) or [, got ${this.peek().type}`);
  }

  private parseParenOrOpenRange(parenStart: number): AstNode {
    // LParen already consumed. Could be:
    // - (expr)           — grouping
    // - (expr..end)      — open range start, exclusive
    // - (expr..end]      — half-open range
    // - (..end)          — unbounded start open range
    ...
    // Parse at BP.Range to stop before .. (so we can detect open range syntax)
    const rangeStart = this.parseExpr(BP.Range);
    ...
    // Not a range — finish parsing any remaining low-BP infix operators
    // (or/and/comparisons/in/between all have BP < BP.Range and were stopped above)
    ...
    if (!this.check(TokenType.RBrace)) {
      do {
        ...
        if (keyTok.type === TokenType.LBracket) {
          ...
          // Multi-word key — collect all name tokens
          key = this.readMultiWordName();
        } else {
          throw new Error(`Expected context key, got ${keyTok.type}`);
        }
  ...
  private readMultiWordName(): string {
    ...
    // FEEL allows +/-/*//' as additional name symbols in context keys
    while (
  ...
  private parseQuantified(): AstNode {
    const start = this.peek().start;
    const quantifier = this.advance().value as 'some' | 'every';
    const bindings: Array<{ name: string; domain: AstNode }> = [];
    ...
    if (!this.check(TokenType.RParen)) {
      do {
        ...
        if (this.match(TokenType.Colon)) {
          // Collect type annotation tokens into a string
  ...
  private parseFunctionCallArgs(callee: AstNode): AstNode {
    ...
    if (!this.check(TokenType.RParen)) {
      do {
        // Peek ahead to check for named parameter: name[-name][ name[-name]]... :
        const saved = this.pos;
        if (this.check(TokenType.Name)) {
          let paramName = this.advance().value;
          // Collect space-, hyphen-, and dot-separated name words (e.g., "Pre-Bureau Risk Category", "Person.Gender")
          while (true) {
            if (
              this.check(TokenType.Minus) &&
              this.pos + 1 < this.tokens.length &&
  ...
  private parseName(): AstNode {
    ...
    // Greedily collect multi-word names by trying to match known patterns.
    // We do a forward-scan and backtrack if the accumulated name isn't known.
    name = this.extendName(name);
  ...
  private extendName(seed: string): string {
    ...
    // Greedily collect consecutive Name/Number tokens (also handles hyphenated names from knownNames)
    let collecting = true;
    while (collecting) {
    ...
    // Handle keyword tokens (of, and, in, or, else) as name-parts when:
    // 1. The resulting name is a known builtin name, OR
    // 2. The resulting name is a user-defined name in knownNames
    const keywordTypes = [
      TokenType.Of,
      TokenType.And,
      TokenType.In,
    ...
    // Keyword token types that can appear as subsequent parts of a multi-keyword builtin name
    ...
    while (extended) {
      ...
      for (const kwType of keywordTypes) {
        ...
        // Check if keyword alone (terminal) completes a known name
        ...
        // Check if candidate is known (builtin or user-defined)
        const isKnown =
        ...
        // Continue collecting plain names/numbers
    ...
    // Always consume 'of' when followed by Name/Number (no standalone operator meaning in FEEL)
    while (this.check(TokenType.Of) && this.pos + 1 < this.tokens.length) {
      ...
      name += ` ${this.advance().value}`; // consume 'of'
      name += ` ${this.advance().value}`; // consume next Name/Number
      while (this.check(TokenType.Name) || this.check(TokenType.Number)) {
        name += ` ${this.advance().value}`;
      }
    }
  ...
  private parseFeelType(): FeelType {
    ...
    // Handle 'function' keyword as a type name
    ...
    if (tok.type === TokenType.Name) {
      ...
      switch (name) {
        ...
        default:
          return { name: 'Unknown', ref: name }; // user-defined type reference
      }
    }

    // "years and months duration" — tok.type is not Name here (already handled above)
    if (tok.value === 'years') {
      this.advance();
      this.expect(TokenType.And);
  ...
  // Parse a comma-separated list of unary tests
  ...
  parseUnaryTests(): AstNode {
    // Unary test: a comma-separated list of tests applied to "?"
    const start = this.peek().start;
...
export function safeParse(
  ...
  try {
    ...
    const posMatch = /at pos (\d+)/.exec(message);
    const pos = posMatch?.[1] ? parseInt(posMatch[1], 10) : 0;
    return { ast: null, errors: [{ message, pos }] };
  }
}
